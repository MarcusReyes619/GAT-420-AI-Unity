...
public class AIUtilityAgent : AiAgent
{
	...
	void Update()
	{
		...
		{ 
			...
			foreach(var utilityObj in utilityObjects)
			{
				utilityObj.score = GetUtilityScore(utilityObj);
				if(utilityObj.score > scoreThreshold && (activeUtilityObject == null || utilityObj.score > activeUtilityObject.score))
				{
					activeUtilityObject = utilityObj;
				}
			}
	...
	IEnumerator UseUtilityCR(AIUtilityObject utilityObject)
	{
		...
		//set animtaion
		animator.SetBool(activeUtilityObject.animationName, true);
		//wait till animation done
		yield return new WaitForSeconds(utilityObject.duration);

		//sets animation back to false
