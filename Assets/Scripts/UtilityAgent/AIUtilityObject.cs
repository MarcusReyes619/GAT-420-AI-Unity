...
public class AIUtilityObject : MonoBehaviour
{
	...
	void LateUpdate()
	{
		// Update the value and position of the meter
		meter.value = score;
		meter.position = transform.position + meterOffset;
	}

	// Retrieve the change associated with a given need type from the registry
	public float GetNeedChange(AIUtilityNeed.Type type)
	{
		return registry.TryGetValue(type, out float value) ? value : 0f;
	}

	// Check if the registry contains a specific need type
